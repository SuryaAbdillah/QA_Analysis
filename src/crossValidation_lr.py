"""
5-Fold Cross Validation for Logistic Regression Win Probability Baseline
========================================================================

Purpose:
- Use Logistic Regression as a simple baseline model for quote win probability prediction.
- Evaluate the model using 5-fold cross-validation.
- Train a final Logistic Regression model on the full dataset after CV evaluation.
- Save fold results, CV summary, final predictions, and trained model.

Why Logistic Regression?
- It is simple and interpretable.
- It provides a baseline to check whether more complex models, such as
  HistGradientBoosting, really improve performance.
- It outputs predicted probabilities through predict_proba(), so it can be used
  for win probability estimation.

How to run:
    python surya_5fold_logistic_regression.py --data ../dataset/df_preprocessed.csv

Alternative path:
    python surya_5fold_logistic_regression.py --data dataset/df_preprocessed.csv

Outputs:
    output/5fold_logistic_regression_results.csv
    output/5fold_logistic_regression_summary.csv
    output/final_logistic_regression_predictions.csv
    output/final_logistic_regression_model.joblib
"""
...
def build_logistic_pipeline(class_weight: str | None = None, c_value: float = 1.0) -> Pipeline:
    """Build Logistic Regression pipeline with imputation, scaling, and one-hot encoding."""
    numeric_features = [col for col in FEATURES if col not in CATEGORICAL_FEATURES]
...
def evaluate_logistic_cv(
    ...
    base_model = build_logistic_pipeline(class_weight=class_weight, c_value=c_value)
    ...
    for fold, (train_idx, test_idx) in enumerate(cv.split(X, stratify_label), start=1):
        ...
        print(
            f"Logistic Regression fold {fold}: "
...
def train_final_logistic_model(
    ...
    """Train final Logistic Regression model on all available data after CV evaluation."""
    model = build_logistic_pipeline(class_weight=class_weight, c_value=c_value)
    model.fit(X, y)
    return model


def save_final_logistic_predictions(df_model: pd.DataFrame, X: pd.DataFrame, model: Pipeline) -> pd.DataFrame:
    ...
    pred_df["logistic_predicted_win_probability"] = model.predict_proba(X)[:, 1]
    pred_df["logistic_predicted_win_probability_pct"] = (
        pred_df["logistic_predicted_win_probability"] * 100
    ).round(2)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="5-fold CV for Logistic Regression win probability baseline.")
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use class_weight='balanced'. For probability estimation, the default is usually better. "
            "Use this only if minority-class recall is more important than probability calibration."
        ),
    )
    parser.add_argument(
        "--c-value",
        type=float,
        default=1.0,
        help="Inverse regularization strength C for Logistic Regression. Default: 1.0",
    )
    ...
    log_output = evaluate_logistic_cv(
        X=X,
        y=y,
    ...
    results_path = os.path.join(OUTPUT_DIR, "5fold_logistic_regression_results.csv")
    summary_path = os.path.join(OUTPUT_DIR, "5fold_logistic_regression_summary.csv")
    log_output.fold_results.to_csv(results_path, index=False)
    log_output.summary.to_csv(summary_path, index=False)

    print("\nLogistic Regression 5-fold summary:")
    print(log_output.summary)

    print("\nTraining final Logistic Regression model on full dataset for predicted probabilities...")
    final_model = train_final_logistic_model(
        X=X,
        y=y,
        class_weight=class_weight,
        c_value=args.c_value,
    )
    final_predictions = save_final_logistic_predictions(df_model, X, final_model)

    final_predictions.to_csv(
        os.path.join(OUTPUT_DIR, "final_logistic_regression_predictions.csv"),
        index=False,
    )
    joblib.dump(final_model, os.path.join(OUTPUT_DIR, "final_logistic_regression_model.joblib"))
